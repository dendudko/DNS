...
# Очевидно, писал код с print-ами, перенаправляю вывод в файл, чтобы много не переделывать :)
...
def analytics_1_1():
    return pd.read_sql('''
        ...
        from t_sales s join t_branches b
        on s."Филиал"=b."Ссылка"
        where lower(b."Наименование") not like '%склад%'
...
def analytics_1_2():
    return pd.read_sql('''
        ...
        from t_sales s join t_branches b
        on s."Филиал"=b."Ссылка"
        where (lower(b."Наименование") like '%склад%'
...
def calculations():
    df = pd.read_sql('''
        ...
        from t_sales s
        join t_products p on s."Номенклатура"=p."Ссылка"
        where lower(p."Наименование") not like '%грузов%'
    ...
    ''')
    conn.commit()
    fill_table_from_file('calculations.csv', one_file=True)
